...
useEffect(() => {
  fetchData();
    ...
    try {
      const snapshot = await EmpDataService.getAllEmp(); 
      if (!snapshot || !snapshot.docs) {
        console.warn("⚠️ No data found or snapshot invalid");
        setUsers([]);
        return;
      }
      ...
      setUsers(usersList);
      console.log("Fetched employees:", usersList);
    } catch (error) {
    ...
    const handleAdd = async(e)=> {
      ...
      // // setFlag(prev => !prev);
      // setMessage("");
      // if(userName===""||userProf===""|| userSalary===""){
      //   setMessage({error:true,msg:'enter all fields'});
      //   return
      // }
      ...
      // } 
      // try{
      //   await EmpDataService.addEmp(newEmp);
